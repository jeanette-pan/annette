...
export default function AddStatusModal({
  ...
  const deleteTemplate = useCallback(async (e: React.MouseEvent, id: string) => {
    e.stopPropagation()
                  ...
                  {templates.length > 0 ? (
                    <div className="flex flex-wrap gap-2 max-h-[7.5rem] overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
                      {templates.map((tpl) => (
                        <div
                          key={tpl.id}
                          className="group/chip relative flex items-center gap-1.5 pl-3 pr-1.5 py-1.5 rounded-full text-xs font-semibold text-gray-700 border border-white/60 shadow-sm hover:shadow-md transition-all duration-150 cursor-pointer"
                          style={{ backgroundColor: tpl.color }}
                          onClick={() => applyTemplate(tpl)}
                        >
                          <span>{tpl.emoji}</span>
                          <span className="max-w-[80px] truncate">{tpl.name}</span>
                          {tpl.isShared && <span className="text-[9px] opacity-50">💚</span>}
                          <button
                            onClick={(e) => deleteTemplate(e, tpl.id)}
                            className="opacity-0 group-hover/chip:opacity-100 w-4 h-4 flex items-center justify-center rounded-full bg-black/10 hover:bg-red-200 text-gray-500 hover:text-red-600 transition-all"
